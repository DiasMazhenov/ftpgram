...
FTPgram по умолчанию не удаляет локальные записи, которых нет в текущем окне
индексации. Включай `TELEGRAM_PRUNE_MISSING=true` только если точно нужно
синхронно чистить SQLite по последней выборке Telegram.
Удаление работает через корзину: файлы можно восстановить, удалить навсегда или
дождаться автоочистки через 30 дней.

После подключения Telegram FTPgram включает live sync: новые, измененные и
удаленные сообщения из `FTPgram Storage` и Telegram «Избранное» обновляют
локальный SQLite-индекс без ручной переиндексации. Ручной reindex остается
fallback для холодного старта и восстановления после сетевых сбоев.
...
На Render FTP выключен по умолчанию (`FTP_ENABLED=false`), потому что Render web
services проксируют HTTP/WebDAV, а отдельный FTP TCP listener получает
платформенные probe/reset соединения и засоряет логи. Локально FTP остается
