...
```bash
TELEGRAM_STORAGE_CHAT="FTPgram Storage"
```

Добавь туда аккаунт, под которым работает MTProto-сессия. Можно указать username
или id канала в `TELEGRAM_STORAGE_CHAT`; так файлы будут храниться отдельно от
личных чатов и подписок.
